...
type Language = "English" | "Hindi" | "Chhattisgarhi" | "Marathi" | "Bengali" | "Tamil";
...
const WELCOME: Record<Language, string> = {
  English: "Namaste. Ask me anything from Charaka Samhita — I will answer with citations.",
  Hindi: "नमस्ते। चरक संहिता से कुछ भी पूछें — मैं उद्धरण सहित उत्तर दूंगा।",
  Chhattisgarhi: "नमस्ते। चरक संहिता ले कुछ भी पूछव — मैं हवाला सहित जवाब देहूं।",
  Marathi: "नमस्ते। चरक संहितेतून काहीही विचारा — मी संदर्भासह उत्तर देईन।",
  Bengali: "নমস্কার। চরক সংহিতা থেকে যেকোনো প্রশ্ন করুন — আমি উদ্ধৃতিসহ উত্তর দেব।",
  Tamil: "வணக்கம். சரக சம்ஹிதாவிலிருந்து எதையும் கேளுங்கள் — மேற்கோளுடன் பதில் சொல்கிறேன்.",
};

const PLACEHOLDER: Record<Language, string> = {
  English: "Query the Charaka Samhita...",
  Hindi: "चरक संहिता से पूछें...",
  Chhattisgarhi: "चरक संहिता ले पूछव...",
  Marathi: "चरक संहिता विचारा...",
  Bengali: "চরক সংহিতা জিজ্ঞাসা করুন...",
  Tamil: "சரக சம்ஹிதாவை கேளுங்கள்...",
};
...
export default function AyurbotDemoPage() {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>(() => [
    { id: "initial-message", role: "assistant", content: WELCOME["English"] },
  ]);
  ...
  useEffect(() => {
    setMessages([{ id: "initial-message", role: "assistant", content: WELCOME[language] }]);
  }, [language]);
  ...
  async function send() {
    ...
    try {
      const res = await fetch(API_URL, {
        ...
        body: JSON.stringify({ query: q, top_k: topK, language }),
      });
  ...
  return (
    <div className="min-h-screen w-full overflow-hidden">
      ...
      <div className="min-h-screen w-full flex flex-col items-start pl-[3%] pt-6 pb-8">
        {/* AyurBot heading */}
        <div
          className="w-[42%] max-w-[850px] min-w-[380px] flex justify-center mb-10"
          style={{ textShadow: "0 2px 16px rgba(62,39,35,0.18), 0 1px 0 rgba(255,255,255,0.4)" }}
        >
          <SplitText
            text="AyurBot"
            tag="h1"
            className="text-7xl font-black tracking-tight text-[#3e2723]"
            delay={80}
            duration={0.9}
            ease="power3.out"
            splitType="chars"
        ...
        <div className="w-[42%] max-w-[850px] min-w-[380px]">
          {/* Main Card */}
            ...
            <div className="px-8 pt-8 pb-4 flex items-start justify-between gap-4">
              ...
              <div className="flex flex-col items-end gap-2">
                ...
                <select
                  ...
                  className="text-xs font-bold px-3 py-1.5 rounded-full bg-[#3e2723]/8 border border-[#3e2723]/20 text-[#3e2723] outline-none focus:ring-2 focus:ring-[#3e2723]/20 cursor-pointer"
                >
                  {LANGUAGES.map((l) => (
                      ...
                      >
                        <div className="text-[15px] leading-relaxed font-medium">
                          <ReactMarkdown
                            ...
                            components={{
                              ...
                              strong: ({ children }) => (
                                <strong className="font-bold text-[#2d5a27]">{children}</strong>
                              ),
                              h3: ({ children }) => (
                                <h3 className="text-lg font-bold text-[#3e2723] mt-4 mb-2 border-b border-[#d7ccc8] pb-1">
                                  {children}
                                </h3>
                              ),
                            }}
                        ...
                        {m.role === "assistant" && m.citations && m.citations.length > 0 && (
                          <div className="mt-4 pt-4 border-t border-[#d7ccc8] space-y-3">
                            <p className="text-[10px] font-black uppercase tracking-[0.15em] text-[#8d6e63]">
                              Source Verification
                            </p>
                            <div className="grid gap-2">
                              {m.citations.map((c) => (
                                <a
                                  ...
                                  target="_blank"
                                  rel="noreferrer"
                                  className="group flex items-center justify-between gap-3 rounded-xl border border-[#d7ccc8] bg-[#f4ece1]/80 px-3 py-2.5 hover:bg-[#2d5a27] hover:border-[#2d5a27] transition-all"
                                >
                                  <div className="text-[11px] font-semibold leading-tight text-[#3e2723] group-hover:text-[#f4ece1]">
                                    <span className="font-bold text-[#2d5a27] group-hover:text-[#a5d6a7]">
                                      #{c.rank}
                                    </span>{" "}
                                    — {prettyCitation(c)}
                                  </div>
